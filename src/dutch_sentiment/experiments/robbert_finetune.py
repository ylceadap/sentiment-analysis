...
def _encode_batch(
    tokenizer: Any, texts: list[str], max_length: int, input_strategy: str = "first"
) -> dict[str, Any]:
    """Tokenize one batch with an explicit first, last, or head-tail truncation policy."""
    if input_strategy == "first":
        return tokenizer(
            texts,
            padding=True,
            truncation=True,
    ...
    if input_strategy not in {"last", "head_tail"}:
        raise ValueError(f"Unsupported RobBERT input strategy: {input_strategy}")

    payload_budget = max_length - int(tokenizer.num_special_tokens_to_add(pair=False))
    if payload_budget < 2:
        raise ValueError("max_length is too small for tokenizer special tokens")
    raw_rows = tokenizer(texts, add_special_tokens=False, truncation=False)["input_ids"]
    encoded_rows: list[dict[str, list[int]]] = []
    for token_ids in raw_rows:
        if input_strategy == "last":
            selected = token_ids[-payload_budget:]
        else:
            # Preserve both the setup and the concluding judgement of long reviews.
            head = (payload_budget + 1) // 2
            tail = payload_budget - head
            selected = (
                token_ids
                if len(token_ids) <= payload_budget
                else token_ids[:head] + token_ids[-tail:]
            )
        input_ids = tokenizer.build_inputs_with_special_tokens(selected)
        encoded_rows.append({"input_ids": input_ids, "attention_mask": [1] * len(input_ids)})
    return tokenizer.pad(encoded_rows, padding=True, return_tensors="pt")


def _collator(tokenizer: Any, max_length: int, input_strategy: str = "first") -> Any:
    ...
    def collate(batch: list[tuple[str, np.ndarray | np.int64]]) -> tuple[dict[str, Any], Any]:
        ...
        encoded = _encode_batch(tokenizer, list(texts), max_length, input_strategy)
...
def _loss_function(
    ...
    ordinal_lambda: float = 0.15,
    focal_gamma: float = 2.0,
) -> Any:
    """Build a configured loss using weights derived only from training labels."""
    import torch
    from torch.nn import functional as torch_functional

    if objective == "multiclass_logistic":
        selected_loss = loss_name or "balanced_cross_entropy"
        if selected_loss == "cross_entropy":
            weights = None
        elif selected_loss in {"balanced_cross_entropy", "mild_class_weights", "focal"}:
            values = balanced_class_weights(training_labels)
            if selected_loss in {"mild_class_weights", "focal"}:
                values = np.sqrt(values)
                values = values / values.mean()
            weights = torch.tensor(values, device=device).float()
        elif selected_loss == "ordinal_aware":
            weights = None
        else:
            raise ValueError(f"Unsupported multiclass loss: {selected_loss}")

        def loss(logits: Any, targets: Any) -> Any:
            integer_targets = targets.long()
            if selected_loss == "focal":
                row_losses = torch_functional.cross_entropy(
                    logits, integer_targets, weight=weights, reduction="none"
                )
                true_probabilities = (
                    torch.softmax(logits, dim=1)
                    .gather(1, integer_targets.reshape(-1, 1))
                    .squeeze(1)
                )
                return (((1.0 - true_probabilities) ** focal_gamma) * row_losses).mean()
            base = torch_functional.cross_entropy(logits, integer_targets, weight=weights)
            if selected_loss != "ordinal_aware":
                return base
            probabilities = torch.softmax(logits, dim=1)
            ranks = torch.arange(len(LABELS), device=logits.device, dtype=logits.dtype)
            # Expected rank distance penalizes Positive<->Negative errors more than adjacent errors.
            distances = torch.abs(ranks.reshape(1, -1) - integer_targets.reshape(-1, 1))
            return base + ordinal_lambda * (probabilities * distances).sum(dim=1).mean()
...
def _predict_logits(
    ...
    loader = DataLoader(
        ...
        collate_fn=_collator(
            ...
            str(settings.get("input_strategy", "first")),
        ),
    )
...
def _train_model(
    *,
    ...
    loader = DataLoader(
        ...
        collate_fn=_collator(
            tokenizer,
            int(training_settings["max_length"]),
            str(training_settings.get("input_strategy", "first")),
        ),
    )
    ...
    loss_function = _loss_function(
        ...
        train_labels,
        device,
        loss_name=training_settings.get("loss"),
        ordinal_lambda=float(training_settings.get("ordinal_lambda", 0.15)),
        focal_gamma=float(training_settings.get("focal_gamma", 2.0)),
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (inputs, targets) in enumerate(loader, start=1):
            ...
            if step % accumulation == 0 or step == len(loader):
                ...
                scaler.update()
                # GradScaler may skip an overflowing FP16 optimizer step; keep LR steps aligned.
...
def verify_bundle(
    ...
    """Verify evidence and optionally require both large trained model directories."""
...
def _register_test_candidates(
    ...
    """Expose both verified RobBERT heads as test-only MLflow Registry entries."""
    names = {
        "multiclass_logistic": "sentiment-robbert-v2-logistic",
        "coral_ordinal": "sentiment-robbert-v2-ordinal",
    }
    for candidate in result["candidates"]:
        objective = str(candidate["objective"])
        name = names[objective]
        weights_stored = (root / "models" / objective / "model.safetensors").is_file()
        try:
        ...
        client.update_registered_model(
            name,
            description=(
                "End-to-end RobBERT v2 candidate evaluated for assignment research; "
                "retained as experiment evidence; only the Logistic result is presentation-selected."
            ),
        )
        model_tags = {
            "governance.role": "test-only",
            "deployment.eligible": "false",
            "presentation.selected": "false",
            "evaluation.scope": str(result["evaluation_scope"]),
            "training.strategy": "end-to-end-finetuning",
            "license": str(config["model"]["license"]),
            "objective": objective,
            "artifact.tier": "reproducible" if weights_stored else "evidence-only",
        ...
        source = (
            f"runs:/{run_id}/models/{objective}" if weights_stored else f"runs:/{run_id}/evidence"
        )
        ...
        version = next(
            (
                ...
                if str(item.run_id) == run_id and str(item.source) == source
            ),
            None,
        )
        if version is None:
        ...
        client.set_registered_model_alias(name, "test-only", str(version.version))
...
def log_existing_bundle(bundle_dir: str | Path, config_path: str | Path) -> dict[str, Any]:
    ...
    if matches:
        ...
        _register_test_candidates(client, run_id, result, config, root)
    ...
    with mlflow.start_run(
        ...
        for candidate in result["candidates"]:
            ...
            model_dir = root / "models" / prefix
            if model_dir.is_dir():
                mlflow.log_artifacts(str(model_dir), artifact_path=f"models/{prefix}")
